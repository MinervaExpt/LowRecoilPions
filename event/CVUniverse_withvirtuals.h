...
class CVUniverse : public PlotUtils::MinervaUniverse {
  ...
  */
  ...
  virtual double GetWeightForOnePiWNeutron() const{
    if (GetTrueEavMinusTpi() > 75) return 1.0;
    if (GetTrueEavMinusTpi() <= 75){
	...
	const std::vector<int> fsParts = GetTrueFSPDGCodes();	
	 ...
	 if (leadingneutron == true && npionp == 1) {
		double weight = 4.941*TMath::Exp(-2.164*TMath::Sqrt(GetMuonPTTrue()));
		return weight;
		//return 1.3 - 0.8729*GetMuonPT() + 0.2825*GetMuonPT()*GetMuonPT();
	 }
 ...
 */
 ...
 virtual double GetTrueEavMinusTpi() const{
 ...
 virtual double GetTrueEavMinusTpiGEV() const{
     std::vector<int> pdgs = GetVecInt("mc_FSPartPDG");
     //int count = std::count(pdgs.begin(), pdgs.end(), 211);
     //double tpi = 0.0; //GetTrueLowestTpiEvent();
     //if (count < 1) tpi = 0.0;
