...
for key in mcFile.GetListOfKeys():
  ...
  if name.find("top") > -1 and name.find(var+"_top") > -1:
    if name.find("pt") > -1: continue;
    hist = key.ReadObj()
...
orderedStack.SetMaximum(dataWithStatErrors.GetMaximum()*1.8) #Make room for annotations above the plot
...
overall.Print(var + "_logycoh_DataMCRatio.png")
