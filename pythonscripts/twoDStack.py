...
if i == int(sys.argv[3]):
	...
	dataHist2d = dataHist2D.GetBinNormalizedCopy()
        dataHistbini = dataHist2d.ProjectionX("e", i,i)
        dataHist = dataHist2D.ProjectionX("e", i,i)
	dataWithStatErrors = dataHist.GetCVHistoWithError().Clone()
	dataHistnorm = dataHistbini.GetCVHistoWithError().Clone()
	...
	mcStack.SetMaximum(mcTotal.GetMaximum() * 1.8) #Make room for annotations above the plot
	mcStack.Draw("HIST")
        mcStack.GetHistogram()
...
#Now fill mcRatio with 1 for bin content and fractional error
	...
	ratio.GetXaxis().SetLabelSize(labelSize)
        ratio.GetXaxis().SetTitle(sys.argv[5])#"Reconstructed Available Energy (MeV)")
	...
	overall.Print(var + "_bin"+str(i) +"_nolog_binnorm_DataMCRatio.png")
