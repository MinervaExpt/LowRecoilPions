...
var = sys.argv[1] #"permichel_pirange" #"NewEAvail" #"pTmu"
dataName = var + "_data"
...
dataFile = TFile.Open("studiesData.root")

mcFile = TFile.Open("studiesMC.root") 
...
for key in mcFile.GetListOfKeys():
  ...
  if name.find("top") > -1 and name.find(var+"_t") > -1:
    ...
    histi = hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone() #hist.GetCVHistoWithError().Clone()
    if newname.find("_top_Kaons_Only") > -1:
    			histi.SetName("Kaons Only")
                        histi.SetLineColor(1321)
                        histi.SetFillColor(1321)
                        hists[0] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_NPi_Plus_Only") > -1:
        		histi.SetTitle("N #pi^{+} Only")
                        histi.SetLineColor(1319)
                        histi.SetFillColor(1319)
			hists[6] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_NPi_and_Other_Mesons") > -1:
                        histi.SetTitle("N #pi^{+} and Mesons")
                        histi.SetLineColor(1323)
                        histi.SetFillColor(1323)
			hists[5] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_top_Other") > -1:
                        histi.SetLineColor(1322)
                        histi.SetFillColor(1322)
			hists[2] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_top_Neutral_Pi") > -1:
                        histi.SetLineColor(1320)
                        histi.SetFillColor(1320)
			hists[4] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_top_QE_like") > -1:
			histi.SetLineColor(1318)
                        histi.SetFillColor(1318)
			hists[3] = histi #hist.GetCVHistoWithError().Clone()#hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
    if newname.find("_top_COH_Pions") > -1:
                        histi.SetLineColor(1317)
                        histi.SetFillColor(1317)
                        hists[1] = histi #hist.GetCVHistoWithError().Clone() #hist.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
...
orderedStack.SetMaximum(dataHistnorm.GetMaximum()*1.5)#dataHistnorm.GetMaximum()*1.8) #Make room for annotations above the plot
orderedStack.Draw("HIST")
histstack = orderedStack.GetHistogram()
histstack.GetXaxis().SetRangeUser(float(sys.argv[3]),float(sys.argv[4]))
orderedStack.Draw("HIST")
...
ratio.GetXaxis().SetTitle(sys.argv[2])#"Reconstructed Pion Range (mm)")
ratio.GetXaxis().SetRangeUser(float(sys.argv[3]), float(sys.argv[4]))#500.,3000.)
...
overall.Print(var + "DataMCRatio_logx_chi2.png")
...
#migration.Draw("colz")
#migcan.Print("range_vs_tpi_mig.png")
...
databkg.Draw("HIST")
migcan.Print("pirange_data_HIST_bkgsub.png")
