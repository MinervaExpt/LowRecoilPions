...
dataFile = TFile.Open("runEventLoopData.root")#runEventLoopData_Sep022022_pmu_reweight.root")
...
for key in mcFile.GetListOfKeys():
  ...
  if name.find("top") > -1 and name.find(var+"_t") > -1:
    ...
    histnorm.GetXaxis().SetTitle(sys.argv[2])
    histnorm.GetYaxis().SetTitle("N Events / MeV")
...
#mcStack.SetMaximum(6000.) #Make room for annotations above the plot
#mcStack.GetHistogram().GetXaxis().SetRangeUser(0, 2000)
#mcStack.GetXaxis().SetRangeUser(0.0, 3000.)
#mcStack.Draw("HIST")
#mcStack.GetHistogram().GetYaxis().SetRangeUser(0, 3000.)
...
dataHistnorm.GetXaxis().SetTitle(sys.argv[2])
#dataHistnorm.Draw("SAME")

#legend = top.BuildLegend(0.75, 0.3, .95, 0.9) #TODO: Explain legend position.  ROOT 6 might even make an intelligent decision about where to place it.
