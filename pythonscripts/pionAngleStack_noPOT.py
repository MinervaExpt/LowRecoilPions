...
dataFile = TFile.Open(sys.argv[3])#"studiesData.root")

mcFile = TFile.Open(sys.argv[2]) #"studiesMC.root") 
...
#orderedStack.SetMaximum(dataWithStatErrors.GetMaximum()*1.8)#dataHistnorm.GetMaximum()*1.8) #Make room for annotations above the plot
orderedStack.Draw("HIST")
orderedStack.GetHistogram().GetYaxis().SetTitle("N Events")
#orderedStack.GetHistogram().GetYaxis().SetRangeUser(0.0,dataWithStatErrors.GetMaximum()*1.8)
orderedStack.Draw("HIST")
#mcStack.Draw("HIST")
print("i am drawing the data")
dataWithStatErrors.SetLineColor(kBlack)
...
orderedStack.Draw("HIST SAMES")
dataWithStatErrors.Draw("SAME")

#overall.Print(var + "_DataMC.png")
...
ratio.GetXaxis().SetTitle("Reconstructed cos(#theta_{#pi^{+}})")#"Reconstructed Pion Range (mm)")
...
overall.Print(var + "_DataMCRatio.png")
