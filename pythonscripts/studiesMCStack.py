...
for key in mcFile.GetListOfKeys():
  ...
  #print name
  if name.find(var+"_top") > -1:
    if name.find("pt_true")> -1: continue
    print name
    hist = key.ReadObj()
...
orderedStack.SetMaximum(dataWithStatErrors.GetMaximum()*1.2) #Make room for annotations above the plot
...
orderedStack.GetHistogram().GetXaxis().SetRangeUser(0.0, 3.0)
orderedStack.Draw("HIST")
#mcStack.Draw("HIST")
