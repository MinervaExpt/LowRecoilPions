...
for key in mcFile.GetListOfKeys():
  ...
  if name.find("top") > -1 and name.find(var+"_t") > -1:
    ...
    if newname.find("_top_COH_Pions") > -1:
          		...
          		hist2 = hist.Clone()
			hist2.Scale(10.)
	                hists[1] = hist2.GetBinNormalizedCopy().GetCVHistoWithError().Clone()
...
orderedStack.SetMaximum(600.)#dataHistnorm.GetMaximum()*1.8) #Make room for annotations above the plot
orderedStack.Draw("HIST")
histstack = orderedStack.GetHistogram()
#histstack.Scale(1000.)
...
overall.Print(var + "DataMCRatio_COHonly_logx_chi2.png")
