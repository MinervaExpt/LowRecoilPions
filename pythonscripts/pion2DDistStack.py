...
hists[0].Divide(hists[0],mcSum)
hists[1].Divide(hists[1],mcSum)
hists[2].Divide(hists[2],mcSum)
hists[3].Divide(hists[3],mcSum)
hists[4].Divide(hists[4],mcSum)
hists[5].Divide(hists[5],mcSum)
hists[6].Divide(hists[6],mcSum)
...
#orderedStack.SetMaximum(10000.)#dataHistnorm.GetMaximum()*1.8) #Make room for annotations above the plot
orderedStack.Draw("HIST")
orderedStack.GetHistogram().GetYaxis().SetTitle("Fraction of Total MC in Bin")
orderedStack.GetHistogram().GetXaxis().SetTitle(sys.argv[2])
orderedStack.Draw("HIST")
#mcStack.Draw("HIST")
...
dataWithStatErrors.SetMarkerSize(0.7)
dataWithStatErrors.SetTitle("Data")
datascale = dataWithStatErrors.Clone()
datascale.Divide(dataWithStatErrors, mcSum.Clone())
datascale.Draw("SAME")
#dataWithStatErrors.Draw("SAME")

mcrat = mcSum.Clone()
mcrat.Divide(mcrat, mcSum)
mcrat.SetLineColor(kRed)
mcrat.SetLineWidth(lineSize)
mcrat.SetMarkerStyle(20)
mcrat.SetMarkerColor(kBlack)
mcrat.SetMarkerSize(0.7)
mcrat.SetTitle("MCtot SAMES")
...
mcTotalWithSys = mcSum.GetCVHistoWithError().Clone()
ratio.Divide(ratio, mcTotalWithSys)
ratio.SetTitle("")
ratio.SetLineColor(kBlack)
ratio.SetLineWidth(lineSize)
ratio.SetTitleSize(0)
#ratio.GetXaxis().SetLimits(0.1, 2400.)
...
ratio.GetXaxis().SetTitle(sys.argv[2])#"Reconstructed cos(#theta_{#pi^{+}})")#"Reconstructed Pion Range (mm)")
#ratio.GetXaxis().SetRangeUser(0.1,2400.)
...
#mcRatio.GetXaxis().SetRangeUser(0.1,2400.)
#mcRatio.GetXaxis().SetLimits(0.1, 2400.)
...
overall.Print(var + "_logx_norm_DataMCRatio.png")
