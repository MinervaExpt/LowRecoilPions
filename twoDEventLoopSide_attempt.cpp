			...
			for(auto& var: vars)
				...
				if (var->GetName() == "Tpi"){
					...
					double eav = var->GetRecoValue(*universe);
                                        ...
                                        var->efficiencyNumerator->FillUniverse(universe, var->GetTrueValue(*universe), weight2);
                                        var->selectedSignalReco->FillUniverse(universe, var->GetRecoValue(*universe), weight2);
...
/*  
  ...
  */
  ...
  std::vector<double> dansPTBins = {0.01, 0.015, 0.020, 0.025, 0.030, 0.035, 0.040, 0.045, 0.05, 0.075, 0.10, 0.125, 0.15, 0.175,0.20, 0.25,0.30,0.35, 0.4, 0.45, 0.50, 0.55, 0.60 , 0.7, 0.80,0.9, 1.,1.1, 1.2, 1.4, 1.6}, 
 ...
 //vars2D.push_back(new Variable2D(*vars[5], *vars[0]));//pTbins vs q2 
