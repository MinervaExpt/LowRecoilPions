...
class Variable2D: public PlotUtils::Variable2DBase<CVUniverse>
{
  ...
  public:
                           ...
                           std::map<std::string, std::vector<CVUniverse*>>& truth_error_bands)
    {
      ...
      // make the error bands in the form MnvResponse wants.  why.
     std::map<const std::string, int> response_bands;
	...
	response_bands[realname] = nuniv;
      ...
      //migration_reco = new PlotUtils::Hist2DWrapper<CVUniverse>((GetName() + "_migration_reco").c_str(), GetName().c_str(), GetRecoBinVecX(), GetRecoBinVecY(), mc_error_bands);

      //migration_true = new PlotUtils::Hist2DWrapper<CVUniverse>((GetName() + "_migration_true").c_str(), GetName().c_str(), GetBinVecX(), GetBinVecY(), mc_error_bands); 
      //m_response = new MinervaUnfold::MnvResponse((GetName() + "_response").c_str(), GetName().c_str(), (Int_t)GetRecoBinVecX().size() - 1, const_cast<const Double_t*>(GetRecoBinVecX().data()), (Int_t)GetRecoBinVecY().size() - 1, const_cast<const Double_t*>(GetRecoBinVecY().data()), (Int_t)GetBinVecX().size() - 1, const_cast<const Double_t*>(GetBinVecX().data()), (Int_t)GetBinVecY().size() - 1, const_cast<const Double_t*>(GetBinVecY().data()), response_bands);
    ...
    {
	std::cout << "Getting Response Objects (inside the function)" << std::endl;
	for(mnv_itr2 = Response.begin(); mnv_itr2 != Response.end(); ++mnv_itr2){
		std::cout << "Getting the Migration objects filled " << std::endl;
    ...
    MinervaUnfold::MnvResponse* m_response;
    ...
    void Write(TFile& file)
    {
      ...
      std::cout << "Writing Histos : " <<  GetNameX() << " vs " << GetNameY() << std::endl;
	...
	biasMCReco->hist->Write();
     ...
     if(efficiencyNumerator)
      ...
      {
    	//efficiencyDenominator->SyncCVHistos();
	...
	if (GetNameY() == "pTmubins"){
      ...
      if(m_response)
      {
	m_response->GetMigrationMatrix()->SetDirectory(&file);
	m_response->GetMigrationMatrix()->Write();
	m_response->GetTruth2D()->SetDirectory(&file);
	m_response->GetTruth2D()->Write();
	m_response->GetReco2D()->SetDirectory(&file);
	m_response->GetReco2D()->Write();
      }
      */
