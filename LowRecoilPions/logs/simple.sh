...
#          /opt/jobsub_lite/bin/jobsub_submit --group minerva -role=Analysis --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --memory=6GB --disk=5GB --expected-lifetime=24h -N 1 -d OUT /pnfs/minerva/scratch/users/sultana/MnvTunev4-3-1_twoDSelection_fullMC_nosystematics_PiReweight_submitted_2023_02_02_15_43_gridtest -e IFDH_CP_MAX_RETRIES=1 -f /pnfs/minerva/resilient/tarballs/sultana_Jan262023_PionReweight_LowRecoilPions.tgz file:///minerva/app/users/sultana/cmtuser/WorkingArea/LowRecoilPions/LowRecoilPions/runEventLoop_job.sh datatest.txt mctest.txt
...
#export BEARER_TOKEN_FILE=$PWD/.condor_creds/minerva_1c2fe1d7d7.use
...
${JSB_TMP}/ifdh.sh cp -D /pnfs/minerva/resilient/tarballs/sultana_Jan262023_PionReweight_LowRecoilPions.tgz ${CONDOR_DIR_INPUT}
...
export JOBSUB_OUT_OUT=out_OUT
mkdir $JOBSUB_OUT_OUT
...
${JSB_TMP}/ifdh.sh log "mengel:$JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT   datatest.txt mctest.txt "
...
echo `date` $JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT datatest.txt mctest.txt >&2
echo `date` $JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT datatest.txt mctest.txt

 $JOBSUB_EXE_SCRIPT datatest.txt mctest.txt
JOB_RET_STATUS=$?

# copy out -d directories

${JSB_TMP}/ifdh.sh cp -D $JOBSUB_OUT_OUT/* /pnfs/minerva/scratch/users/sultana/MnvTunev4-3-1_twoDSelection_fullMC_nosystematics_PiReweight_submitted_2023_02_02_15_43_gridtest
