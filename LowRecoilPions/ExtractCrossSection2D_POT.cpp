...
void Plot(PlotUtils::MnvH2D& hist, const std::string& stepName, const std::string& prefix)
{
  ...
  std::vector<std::string> groupnames = {"Tune","Geant", "Response", "Detector", "Flux", "Muon", "Genie Interaction Model", "FSI Model", "Diffractive", "PhysicsModel", "NonResPi"};
  //#include "util/SafeROOTName.h"
  ...
  */
  ...
  for(const auto& prefix: crossSectionPrefixes)
  {
	...
	if (prefix.find(name) != std::string::npos and prefix.find("pTmubins") != std::string::npos){
      ...
      auto crossSection = normalize(unfolded, flux, nNucleons->GetVal(), dataPOT);//totalPOT); //dataPOT if you are going playlist by playlist
      ...
      normalize(simEventRate, flux, nNucleons->GetVal(),mcPOT); //mcPOT if you go playlist by playlist
