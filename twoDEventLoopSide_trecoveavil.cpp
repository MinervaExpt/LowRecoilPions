	...
	if (cutResults.all()){
		...
		//std::cout << "Printing relevant vars: recotpi " << recotpi << std::endl;
   		for(auto& var: vars) {
			...
			else if(endOfPrefix3 != std::string::npos &&  endOfPrefix2 == std::string::npos && endOfPrefix1 == std::string::npos){
		...
		if(isSignal){
			...
			for(auto& var: vars)
				...
				if (var->GetName() == "Tpi"){
					var->migration->FillUniverse(universe, recotpi, var->GetTrueValue(*universe), weight2);
                           		...
                           		double eavtpidiff = eav - recotpi;
					if (recotpi > eav) eavtpidiff = 0.0;
					var->migration->FillUniverse(universe, eavtpidiff, var->GetTrueValue(*universe), weight2);
                                        var->efficiencyNumerator->FillUniverse(universe, var->GetTrueValue(*universe), weight2);
                                        var->selectedSignalReco->FillUniverse(universe, eavtpidiff, weight2);
				}
				else if (var->GetName() == "texp"){
			...
			for(auto& var: vars2D)
				...
				if(endOfPrefix1 != std::string::npos){
                          		...
                          		double eavtpidiff = eav - recotpi;
                          		if (recotpi > eav) eavtpidiff = 0.0;
					var->selectedSignalReco->FillUniverse(universe, eavtpidiff, var->GetRecoValueY(*universe), weight2);
                                        var->efficiencyNumerator->FillUniverse(universe, var->GetTrueValueX(*universe), var->GetTrueValueY(*universe), weight2);
					var->FillResponse(eavtpidiff, var->GetRecoValueY(*universe),var->GetTrueValueX(*universe), var->GetTrueValueY(*universe),universe->ShortName(),weight2,unv_count);
				}
				else if (endOfPrefix2 != std::string::npos && endOfPrefix1 == std::string::npos){
			...
			int bkgd_ID = -1;
				...
				const std::string keyName = var->GetName();
			        ...
			        if(endOfPrefix1 != std::string::npos){
                                        ...
                                        double eavtpidiff = eav - recotpi;
                                        if (recotpi > eav) eavtpidiff = 0.0;
				        (*var->m_backgroundHists)[bkgd_ID].FillUniverse(universe, eavtpidiff, var->GetRecoValueY(*universe), weight2);
		...
		for(auto& var: vars)
      		...
      		for(auto& var: vars2D)
      		{
				...
				double eav = var->GetRecoValueX(*universe);
                                double eavtpidiff = eav - recotpi;
                                if (recotpi > eav) eavtpidiff = 0.0;
				var->dataHist->FillUniverse(universe, eavtpidiff, var->GetRecoValueY(*universe), 1);
			}
			else if(endOfPrefix2 != std::string::npos && endOfPrefix1 == std::string::npos){
...
/*  
  ...
  */
  ...
  std::vector<double> tpibins = {1., 10., 15., 20., 24., 28., 32., 36., 40., 46., 52.,60., 70., 80., 100., 125.,150., 175., 200., 225., 250., 275., 300., 325., 350., 400., 500., 700., 1000.}; 
  ...
  std::vector<double> tpibinsGEV = {.001, .010, .015, .020, .024, .028, .032, .036, .040, .046, .052,.060, .070, .080, .100, .125,.150, .175, .200, .225, .250, .275, .300, .325, .350, .400, .500};
