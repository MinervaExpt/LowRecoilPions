...
class TestDefinition(testbase.SamdevTest):
    ...
    def test_snapshot_info(self):
        info = self.samweb.snapshotInfo(snapshot_id=1)
        self.assertEquals(info['snapshot_id'], 1)
        self.assertEquals(info['defname'], defname)

        info = self.samweb.snapshotInfo(defname=defname)
        self.assertEquals(info['snapshot_id'], 1)
        self.assertEquals(info['defname'], defname)
