...
{
  char *mparalocation = std::getenv("UTILPATH");//"MPARAMFILESROOT");
  //std::string f = std::string(mparalocation) + "/data/Reweight/lowQ2pi_weights.root";
  //char *minstalldir = std::getenv("INSTALL_DIR");
  //std::cout << "Printing directory of lowQ2 root file " << minstalldir << std::endl;
  //char *mweightsfilelocation = std::getenv("WEIGHTFILEPATHROOT");
  //std::cout << "Printing the path of the weight file " << std::string(mweightsfilelocation) << std::endl;
  std::string f = std::string(mparalocation) +"/lowQ2pi_weights.root";///minerva/app/users/sultana/cmtuser/WorkingArea/LowRecoilPions/LowRecoilPions/util/lowQ2pi_weights.root";
  read(f);
}
