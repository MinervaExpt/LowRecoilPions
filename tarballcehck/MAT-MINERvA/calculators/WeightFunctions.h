...
virtual double GetLowQ2PiWeight(std::string channel) const {
  ...
  if( channel == "MENU1PI" )
  {
    ...
    else if( !PlotUtils::IsCCNucleonPion(*this) ) return 1.;
    else{
      //std::cout << " Printing Arachne Link for This LowQ2 Weight Event " << PrintTrueArachneLink() << std::endl;
      //double q2true = GetQ2True() * 1e-6;
      //double weight = PlotUtils::weight_lowq2pi().getWeight(GetQ2True() * 1e-6 /*GeV^2*/,
      //                                             channel, variation, GetInt("mc_targetNucleus"));
      //std::cout << "This Low Q2 Weight is being called for Q2 : " << q2true << " with weight: "  << weight << std::endl;
      return PlotUtils::weight_lowq2pi().getWeight(GetQ2True() * 1e-6 /*GeV^2*/,
                                                   channel, variation, GetInt("mc_targetNucleus"));
    }
  }
  else if (!PlotUtils::IsCCRes(*this) )
    return 1.;
  else
    return PlotUtils::weight_lowq2pi().getWeight(GetQ2True() * 1e-6 /*GeV^2*/,
                                                 channel, variation, GetInt("mc_targetNucleus"));
